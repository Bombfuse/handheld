...
impl Mmap {
    ...
    /// Creates an Mmap backed by an existing heap allocation.
    /// The Mmap does NOT own the memory — it won't unmap on drop.
    pub fn from_heap_ptr(ptr: *mut u8, len: usize) -> Self {
        Self {
            aspace: None,
            range: Range {
                start: VirtualAddress::new(ptr as usize),
                end: VirtualAddress::new(ptr as usize + len),
            },
        }
    }
