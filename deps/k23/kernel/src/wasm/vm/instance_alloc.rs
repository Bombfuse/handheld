...
impl InstanceAllocator for PlaceholderAllocatorDontUse {
    ...
    unsafe fn allocate_memory(
        ...
        // Use minimum memory size + small guard instead of MEMORY_MAX (4GB)
        let allocation_bytes = round_usize_up_to_host_pages(minimum);
        let offset_guard_bytes = round_usize_up_to_host_pages(crate::arch::PAGE_SIZE);
        let request_bytes = allocation_bytes + offset_guard_bytes;
        tracing::info!("allocate_memory: min={} total={}", minimum, request_bytes);

        // Allocate WASM memory on the kernel heap (not as USER-mapped Mmap)
        // to avoid demand-paging issues when JIT code runs in S-mode
        let align = crate::arch::PAGE_SIZE;
        let layout = core::alloc::Layout::from_size_align(request_bytes, align)
            .map_err(|_| anyhow::anyhow!("bad layout"))?;
        let ptr = unsafe { alloc::alloc::alloc_zeroed(layout) };
        if ptr.is_null() {
            anyhow::bail!("Failed to allocate WASM memory ({request_bytes} bytes)");
        }
        let mmap = Mmap::from_heap_ptr(ptr, request_bytes);
        tracing::debug!("  WASM memory at {:p} ({} bytes)", ptr, request_bytes);
    ...
    unsafe fn allocate_table(
        ...
        tracing::info!("allocate_table: min={} max={:?} reserve={}", table.limits.min, maximum, reserve_size);
