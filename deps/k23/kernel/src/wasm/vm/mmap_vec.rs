...
impl<T> MmapVec<T> {
        ...
        T: Clone,
    {
        if slice.is_empty() {
            ...
            tracing::debug!("MmapVec::from_slice: {} elements, {} bytes", slice.len(), core::mem::size_of_val(slice));
            let mut this = Self::new_zeroed(aspace.clone(), slice.len())?;
            tracing::debug!("MmapVec::new_zeroed done, extending...");
            this.extend_from_slice(&mut aspace.lock(), slice);
            tracing::debug!("MmapVec::from_slice done");
            Ok(this)
        }
    }
